...
app.get('/data', async function (req, res) {
    try {
        ...
        if (_tac == null) {
            ...
            tac = _tac + tac_;
            var result = await get_signature_for_get(uid, tac);
            var aweme_list = await get_data_all(uid, result.signature);
            var data_json = {
                "code": 0,
                "total": aweme_list.length,
...
app.listen(5000, function () {
    console.log('启动服务端口5000,耐心等待几秒,等待初始化完毕....')
})
...
async function get_signature_for_get(uid, tac) {
    ...
    var base64_tac = Buffer.from(tac, 'utf-8').toString('base64');
    // 核心
    var signature = await page.evaluate(get_signature, uid, base64_tac);
...
async function get_data(uid, signature, max_cursor) {
    try {
...
async function get_data_all(uid, signature) {
    var max_cursor = 0
    var aweme_list = [];
    // 循环max_cursor 得到所有作品信息
    while (max_cursor !== null) {
        var temp = await get_data(uid, signature, max_cursor);
        if (temp == null) {
            break
        }
        aweme_list.push.apply(aweme_list, temp.aweme_list);
        if (temp.has_more == true) {
            max_cursor = temp.max_cursor;
        } else {
            max_cursor = null;
        }
    }
